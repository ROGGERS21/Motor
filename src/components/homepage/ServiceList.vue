<template>
    ...
    <div>
        <SectionHeading title="Our Latest Service" description="Service"/>
    </div>
    <div class="flex flex-col md:flex-row items-center justify-center gap-6 py-10 md:py-20">
        <div v-for="(event, index) in events"
        :key="index"
        class="flex flex-col justify-center items-center w-full md:w-60">
            ...
            <div class="pt-4 text-center">
                <h2 class="text-lg font-semibold text-green-600 mt-2">{{ event.title }}</h2>
                <p class="text-md text-gray-500">{{  event.description }}</p>
            </div>
         </div>
